...
fn body_view(
    ...
    let chips: Vec<_> = if branches.is_empty() {
        ...
        branches
            .iter()
            .map(|name| super::ref_chip(name, s.onto == *name, theme, |st, picked| {
                if let Some(rs) = rebase_state_mut(st) { rs.onto = picked; rs.error = None; }
            }).boxed())
            .collect()
    };

    let warning = label(
        "this rewrites your current branch — push will require --force-with-lease",
    )
    .brush(theme.warn)
    .text_size(11.0);
...
fn run_rebase(st: &mut AppState) {
    ...
    if crate::app::is_demo_repo(&repo_path) {
        if let Some(s) = rebase_state_mut(st) { s.error = Some("demo mode".into()); }
        return;
    }
    if onto.is_empty() {
        if let Some(s) = rebase_state_mut(st) { s.error = Some("pick a base".into()); }
        return;
    }
    ...
    match git::ops::rebase(&repo_path, &onto) {
        ...
        Err(e) => {
            if let Some(s) = rebase_state_mut(st) { s.error = Some(format!("{e:#}")); }
        }
    }
}

fn rebase_state(state: &AppState) -> Option<&RebaseModalState> {
    match &state.modal { Some(Modal::Rebase(s)) => Some(s), _ => None }
}

fn rebase_state_mut(state: &mut AppState) -> Option<&mut RebaseModalState> {
    match &mut state.modal { Some(Modal::Rebase(s)) => Some(s), _ => None }
}
